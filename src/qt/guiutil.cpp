...
{
    // Convert zong:// to zong:
    //
    //    Cannot handle this later, because bitcoin:// will cause Qt to see the part after // as host,
    //    which will lower-case it (and thus invalidate the address).
    if(uri.startsWith("zong://"))
    {
        uri.replace(0, 12, "zong:");
    }
...
{
    return GetSpecialFolderPath(CSIDL_STARTUP) / "Zong.lnk";
}
...
{
    return GetAutostartDir() / "Zong.desktop";
}
...
HelpMessageBox::HelpMessageBox(QWidget *parent) :
    QMessageBox(parent)
{
    header = tr("Zong-Qt") + " " + tr("version") + " " +
        QString::fromStdString(FormatFullVersion()) + "\n\n" +
        tr("Usage:") + "\n" +
        "  Zong-qt [" + tr("command-line options") + "]                     " + "\n";
